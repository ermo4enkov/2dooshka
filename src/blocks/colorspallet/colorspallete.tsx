...
const PALLETEXAMPLE = styled.div`
  ...
  width: 70px;
  height: 70px;
  background-color: ${(props: ColorPalletProps) =>
...
function Pallet(props: ColorPalletProps): JSX.Element {
  return (
    <div>
      <PALLETEXAMPLE color={props.color} />
...
export class ColorsPallet extends React.Component<ColorPalletProps> {
  ...
  render() {
    const colorsCollection = colorsArray.map((color, index) => {
      return <Pallet key={index} color={color} />;
    });

    return (
      <div className="storybook__element">
        <h1 className="headerText">Colors</h1>
        <div className="colors__wrap">{colorsCollection}</div>
      </div>
    );
  }
}
